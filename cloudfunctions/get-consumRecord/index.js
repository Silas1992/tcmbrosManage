...
exports.main = async (event,context) => {
  ...
  if (event.name && /^(\s*\S+\s*)+$/.test(event.name)) {
    // title不为空字符串
    query.goods_name = app.database().RegExp({
      regexp: event.name,
      options: 'i', //大小写不区分
    })
  }
  ...
  if(event.uTime){
    query.timestamp = _.gt(event.uTime)
  }
  //电话号码模糊搜索
  if (event.tel) {
    // title不为空字符串
  ...
  if(!event.page){
  //   const result = await db.collection('buy_record').where(query).get()
  //   return result
    //先取出集合记录总数
    ...
    const result = await db.collection('buy_record').skip(depagesize)
    .limit(event.pagesize).orderBy(orderstr, ordertype).where(query).get()
    return result
  }
