...
exports.main = async (event,context) => {
  ...
  if (event.name && /^(\s*\S+\s*)+$/.test(event.name)) {
    // title不为空字符串
    query.member_name = app.database().RegExp({
      regexp: event.name,
      options: 'i', //大小写不区分
    })
  }
  ...
  if(!event.page){
    ...
    for(let i = 0;i<batchTimes;i++){
      const promise = db.collection('order').skip(i*MAX_LIMIT).limit(MAX_LIMIT).where(query).orderBy('create_time','desc').get()
      tasks.push(promise)
    }
    ...
    const result = await app.database().collection('order').skip(depagesize).limit(event.pagesize)
