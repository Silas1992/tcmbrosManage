...
exports.main = async (event, context) => {
  ...
  //名称模糊搜索
  if (event.activity && /^(\s*\S+\s*)+$/.test(event.activity)) {
    // title不为空字符串
    query.activity = cloud.database().RegExp({
      regexp: event.activity,
      options: 'i', //大小写不区分
    })
  }
  if(event.type){
  ...
  if(!event.page){
    const result = await db.collection('activity_rules').where(query).get({
    ...
    const result = await db.collection('activity_rules').skip(depagesize).limit(event.pagesize)
